...
- `fetch-pdf --ingest` sends only the fetched PDF into the ingest pipeline. If `--out-dir` is also supplied, the PDF is saved there but ingested through an isolated temporary single-file inbox, so unrelated PDFs in that directory are not processed.
