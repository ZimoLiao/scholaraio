...
4. 看 `README.md`、`README_CN.md`、`AGENTS.md`、`CLAUDE.md`、`AGENTS_CN.md`、`.qwen/QWEN.md`
...
### 2.6 `.qwen/QWEN.md` 已经提供了一个更薄的宿主上下文样板
...
- 比 `AGENTS.md` / `CLAUDE.md` 明显更薄
- 聚焦 Qwen 宿主本身该知道什么
- 需要更多上下文时再回到 `AGENTS.md`

结论：

- “宿主特定上下文可以更薄”这件事已经被仓库自身证明过
- 后面如果要改写 `AGENTS.md` / `CLAUDE.md`，应该学它的分层思路
- 但不能机械照搬成纯跳转页，因为 `AGENTS` / `CLAUDE` 还承担更重的长期契约

### 2.7 `setup` 的关键上手策略已经有行为测试守着

我继续核对了 `tests/test_setup.py`，可以确认：

- PDF 解析器推荐逻辑
- wizard 里的解释顺序
- 如何转述 MinerU / Docling 检测结果

这些并不只是“写得长一点的说明文字”，而是已经被测试锁住的用户体验规则。

结论：

- `setup` 不能简单归类成“太长的参考手册”
- 它后面如果要瘦身，应该优先拆部署附录、长样例和高级字段细表
- 核心策略本身应该继续留在主 skill
...
- 如果要改写 `AGENTS.md` / `CLAUDE.md` / `.qwen/QWEN.md`，目标应该是“薄目录 + 必要契约”，不是纯跳转页
- 如果要改写大 skill，应该先区分 reference-heavy 和 strategy-heavy；`document` / `draw` 与 `setup` 不该按同一把刀处理
...
5. `.qwen/QWEN.md` 已经说明宿主特定上下文可以更薄，但 `AGENTS` / `CLAUDE` 仍然需要承担更重的长期契约
6. `setup` 的一部分长内容其实是核心上手策略，不该在后续瘦身时一并外置
...
- agent 常驻文档的目标是“薄目录 + 必要契约”
- skill 瘦身必须分类型治理，而不是统一按行数裁剪
